...
contract RegisterPool is Script {
     ...
     */
    function run(address adapter, bytes calldata poolParams, address quoteToken) external {
        // ============ Load Configuration ============
        ...
        require(registry.isAdapterApproved(adapter), "Adapter not approved. Run admin:approve-adapter first");
        ...
        require(adapterContract.supportsPool(poolParams), "Pool not supported by adapter");
        ...
        require(quoteToken == token0 || quoteToken == token1, "Quote token must be one of the pool tokens");
